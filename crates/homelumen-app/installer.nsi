; cargo-packager's own NSIS template, copied verbatim from
; https://github.com/crabnebula-dev/cargo-packager/blob/main/crates/packager/src/package/nsis/installer.nsi
; (fetched 2026-09-06), with only the two `ManifestDPIAware` lines below
; added: cargo-packager does not declare the installer DPI-aware on its
; own, which is what blurred it on a high-density screen. If cargo-packager
; is ever upgraded, diff this file against the current upstream one and
; carry these two lines forward; nothing else here should ever be hand-edited.
